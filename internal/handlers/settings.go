...
func SettingsHandler(db *service.StudentService, b *telebot.Bot) {
	b.Handle("/settings", func(c telebot.Context) error {
		...
		if !db.Exists(userID) {
			return c.Send("Для начала, используйте /register для регистра��ии")
		}

		sessionsStore.Set(userID, NewUserSession(StateWaitingSetting))
		return c.Send("Выберите необходимую настройку ниже👇", SettingsMenu)
	})
}

func handlerSettingsCommand(db *service.StudentService) func(telebot.Context) error {
	return func(c telebot.Context) error {
		...
		sessions[userID] = NewUserSession("waiting_setting")

		return c.Send("Выберите необходимую настройку ниже👇", SettingsMenu)
	}
}

func handleSetting(db *service.StudentService) func(telebot.Context) error {
	return func(c telebot.Context) error {
		userID := c.Sender().ID
		...
		case "waiting_setting":
			...
			switch setting {
			case "Сменить имя":
				session.State = "waiting_new_name"
		...
		case "waiting_new_name":
			...
			if err := db.UpdateName(userID, newName); err != nil {
				endSession(userID)
				return c.Send("Не удалось обновить имя. Попробуйте /settings заново.")
			}
